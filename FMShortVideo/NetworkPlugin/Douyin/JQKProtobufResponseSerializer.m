...
- (id)responseObjectForResponse:(NSURLResponse *)response
    ...
    if (data.length > 0) {
        NSLog(@"收到protobuf数据 %@", data);
        BOOL wirteSuccess = [data writeToFile:[self protobufPath] atomically:YES];
        NSLog(@"保存protobuf数据 %@, path = %@", wirteSuccess ? @"成功" : @"失败", [self protobufPath]);
